...
OUT=$($GF_BIN scan --min-bytes 100 --only posix || true)
...
OUT=$($GF_BIN scan --min-bytes 100 --only posix --verbose || true)
if ! echo "$OUT" | grep -q "ghostshm_test_noise"; then
...
# Normally we can't easily force it without root, BUT `ghostshm` checks `/proc` iterate.
# If we run as non-root on a system with other users/root processes, it *will* be partial access.
# The user's environment showed `PARTIAL_PROC_ACCESS=true` in previous runs.
# So we can just check if classification is likely_orphan when threshold=0.

echo "Scanning private posix file..."
OUT=$($GF_BIN scan --only posix --threshold 0s --min-bytes 0 || true)
echo "$OUT"
...
# Using C helper
gcc tests/helpers/create_shm_key.c -o tests/helpers/create_shm_key
